  ...
  AddToCartHandler.prototype.updateButton = function (e, fragments, cart_hash, $button) {
    ...
    if ($button) {
      ...
      if (fragments && ! wc_add_to_cart_params.is_cart && $button.parent().find( '.added_to_cart' ).length === 0) {
        var parentProduct = $button.closest('.product-card');
        var productImage = $(parentProduct).find('.img-block img').attr('src');
        var productTitle = $(parentProduct).find('.product-name a').text();
